...
# ============================================
# DATENMODELL – für mehrzeilige Stammformen
# ============================================

@dataclass
class VokabelEintrag:
    """Repräsentiert eine Hauptnummer mit mehreren Zeilen in der mittleren Spalte."""
    position: int                       # Nummer der Vokabel
    griechisch: str                      # Hauptwort (linke Spalte)
    stammzeilen: List[str] = field(default_factory=list)   # Zeilen der mittleren Spalte
    bedeutungen: List[str] = field(default_factory=list)   # zugehörige Bedeutungen (gleiche Länge)
    gefunden: bool = False                # ob mindestens eine Bedeutung gefunden wurde (für Statistik)
...
class OmegaWortschatz:
    ...
    def _normalize(self, text: str) -> str:
        ...
        text = re.sub(r'[\u0300-\u036f]', '', text)  # entfernt alle kombinierenden Akzente
        return text
...
# ============================================
# PDF-PARSER – mit Tabellenerkennung und mehrzeiligen Einträgen
# ============================================
...
class VokabelPDFParser:
    ...
    def _suche_bedeutungen(self, eintrag: VokabelEintrag):
...
class PDFGenerator:
    ...
    def erstelle_pdf(eintraege: List[VokabelEintrag]) -> BytesIO:
        ...
        for e in eintraege:
            ...
            bed_text = "<br/>".join(b if b else " " for b in e.bedeutungen)

            stamm_para = Paragraph(stamm_text, cell_style) if stamm_text else ""
            bed_para = Paragraph(bed_text, cell_style) if bed_text else ""
...
def main():
    ...
    if uploaded_file:
        ...
        if eintraege:
                ...
                with st.expander(f"{e.position}. {e.griechisch}", expanded=False):
                    ...
                    if neues_griechisch != e.griechisch:
                        ...
                        for i, stamm in enumerate(e.stammzeilen):
                            ...
                            if suchwort:
                                bedeutungen, gef, _ = parser.omega.finde_bedeutung(suchwort)
                                if gef and bedeutungen:
                                    e.bedeutungen[i] = "; ".join(bedeutungen)
                                    e.gefunden = True
                                else:
                                    e.bedeutungen[i] = ""
            ...
            st.markdown("---")
            st.subheader("👁️ Vorschau (erste 3 Hauptvokabeln)")
            for e in korrigierte[:3]:
                st.markdown(f"**{e.position}. {e.griechisch}**")
                for stamm, bed in zip(e.stammzeilen, e.bedeutungen):
                    st.markdown(f"- {stamm} → {bed if bed else '⚠️'}")
                st.markdown("---")
        ...
        **Beispiel für die erwartete Tabellenstruktur:**  
        ...
        |-----|------------|-------------|-----------|
        | 1   | ὁ ἀνήρ     | τοῦ ἀνδρός  | Mann      |
        | 2   | ὅς         | ἤ, ὅ        | der, welcher |
        |     |            | ὄς ἄν       | wer auch immer |
        | 3   | μάλα       | μᾶλλον      | sehr      |
        |     |            | μάλιστα     | am meisten |

        Die App gruppiert die Zeilen ohne Nummer automatisch zur vorherigen Hauptnummer.
        """)
