...
@dataclass
class TabellenZeile:
    """Eine Zeile der extrahierten Tabelle - exakte Kopie der Originalzeile"""
    linke_spalte: str      # Inhalt der linken Spalte
    mittlere_spalte: str   # Inhalt der mittleren Spalte
    rechte_spalte: str     # Inhalt der rechten Spalte (bereits vorhanden oder leer)
    original_zeile: str    # Die komplette Originalzeile (für Debugging)
...
# ============================================
# PDF-PARSER – EXAKTE KOPIE DER ORIGINALSTRUKTUR
# ============================================
...
class VokabelPDFParser:
    ...
    def parse_pdf(self, pdf_file) -> List[TabellenZeile]:
        """Extrahiert alle Zeilen aus dem PDF - EXAKT wie im Original"""
        zeilen = []
        with pdfplumber.open(pdf_file) as pdf:
            for seite in pdf.pages:
                ...
                for line in text.split('\n'):
                    line = line.strip()
                    if not line:
                        # Leerzeilen werden übersprungen (oder du könntest sie mit "" speichern, falls gewünscht)
                        continue
                    
                    # Originalzeile speichern (für Debugging)
                    original = line
                    
                    # Aufteilung in Spalten (Tabulator oder mehrere Leerzeichen)
                    ...
                    if len(parts) >= 3:
                        ...
                        # Nur eine Spalte – versuchen zu erkennen, ob es eine Nummer enthält
                    ...
                    zeilen.append(TabellenZeile(
                        ...
                        original_zeile=original
                    ))
        return zeilen
    
    def fuelle_bedeutungen(self, zeilen: List[TabellenZeile]) -> List[TabellenZeile]:
        """
        Ergänzt NUR leere rechte Spalten mit Bedeutungen.
        - Bei Zeilen mit Nummer: Bedeutung des Hauptworts (linke Spalte)
        - Bei Zeilen ohne Nummer: Bedeutung des Wortes in der mittleren Spalte
        Vorhandene Bedeutungen bleiben unverändert.
        """
        for zeile in zeilen:
            # Wenn bereits eine Bedeutung vorhanden (auch Leerzeichen), überspringen
            if zeile.rechte_spalte and zeile.rechte_spalte.strip():
                continue
            
            # Prüfen, ob die Zeile eine Nummer enthält
            ist_neue_vokabel = bool(self.num_pattern.match(zeile.linke_spalte))
            
            # Suchtext je nach Zeilentyp
            if ist_neue_vokabel:
                suchtext = zeile.linke_spalte
            else:
                suchtext = zeile.mittlere_spalte
            
            # Griechische Wörter extrahieren
            griechwoerter = self.greek_pattern.findall(suchtext)
            if not griechwoerter:
                continue
            
            # Erstes griechisches Wort als Suchbegriff
            suchwort = griechwoerter[0]
            bedeutungen, gefunden, _ = self.omega.finde_bedeutung(suchwort)
            if gefunden:
                zeile.rechte_spalte = '; '.join(bedeutungen)
            # Wenn nicht gefunden, bleibt die Zelle leer (kein "?" mehr)
        
        return zeilen


# ============================================
# PDF-GENERATOR – ERSTELLT PDF MIT EXAKTER KOPIE
# ============================================
...
class PDFGenerator:
    ...
    def erstelle_pdf(zeilen: List[TabellenZeile]) -> BytesIO:
        ...
        # Griechische Schriftart registrieren
        try:
            pdfmetrics.registerFont(TTFont('DejaVu', 'DejaVuSans.ttf'))
            schrift = 'DejaVu'
        except:
            schrift = 'Helvetica'
            st.warning("⚠️ Schriftart 'DejaVuSans.ttf' nicht gefunden. Griechische Zeichen werden möglicherweise nicht korrekt dargestellt.")
        ...
        for z in zeilen:
            # EXAKTE Kopie der Originaldaten
...
def main():
    ...
    st.markdown("""
    Lade eine Vokabelliste im PDF-Format hoch.  
    Die App **kopiert die Tabelle exakt** und ergänzt nur **leere** Zellen in der rechten Spalte mit Bedeutungen.
    """)
    ...
    with st.sidebar:
        ...
        **So funktioniert die App:**  
        - Die Tabelle wird **genau so übernommen**, wie sie im PDF ist.  
        - **Vorhandene Bedeutungen** in der rechten Spalte bleiben unverändert.  
        - **Leere rechte Spalten** werden gefüllt:  
          * Bei Zeilen mit **Nummer** → Bedeutung des Hauptworts (linke Spalte).  
          * Bei Zeilen **ohne Nummer** → Bedeutung des Wortes in der mittleren Spalte (z.B. μᾶλλον, μάλιστα).  
        - Wird keine Bedeutung gefunden, bleibt die Zelle **leer**.
    ...
    if uploaded_file:
        ...
        if zeilen:
            ...
            # Statistik
            vorhanden = sum(1 for z in zeilen if z.rechte_spalte and z.rechte_spalte.strip())
            ergaenzt = sum(1 for z in zeilen if z.rechte_spalte and z.rechte_spalte.strip() and not z.rechte_spalte.startswith('?') and z not in [x for x in zeilen if x.rechte_spalte and x.rechte_spalte.strip()])
            
            col1, col2, col3 = st.columns(3)
            col1.metric("Zeilen insgesamt", len(zeilen))
            col2.metric("Bereits vorhanden", vorhanden)
            col3.metric("Neu ergänzt", ergaenzt)
            ...
            for i, z in enumerate(zeilen):
                with st.expander(f"Zeile {i+1}: {z.mittlere_spalte[:30] if z.mittlere_spalte else '(leer)'}...", expanded=False):
                    ...
                    with col_a:
                        ...
                        st.markdown(f"**Originalzeile:** {z.original_zeile}")
                    with col_b:
            ...
            with col2:
                if st.button("📊 CSV exportieren"):
                    df = pd.DataFrame([{
                        ...
                        'Originalzeile': z.original_zeile
            ...
            preview_df = pd.DataFrame([{
                ...
                'Status': '✓' if z.rechte_spalte and z.rechte_spalte.strip() else ' '
