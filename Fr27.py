...
@dataclass
class TabellenZeile:
    """Eine Zeile der extrahierten Tabelle"""
    linke_spalte: str      # Inhalt der linken Spalte (kann leer sein)
    mittlere_spalte: str   # Inhalt der mittleren Spalte (Stammformen / Zusatz)
    rechte_spalte: str = "" # Wird später mit Bedeutung gefüllt
    position: int = 0       # Zeilennummer im Original
    ist_neue_vokabel: bool = False  # True, wenn Zeile mit Nummer beginnt
    hauptwort: str = ""     # Das zugehörige Grundwort (aus der ersten Zeile eines Blocks)

# ============================================
# OMEGA-WORTSCHATZ (VOLLSTÄNDIG)
# ============================================
...
class VokabelPDFParser:
    ...
    def __init__(self):
        ...
        # Griechischer Unicode-Bereich
        self.greek_pattern = re.compile(r'([\u0370-\u03FF\u1F00-\u1FFF]+)')
        # Muster für Nummer am Zeilenanfang
        self.num_pattern = re.compile(r'^(\d+)')
    
    def parse_pdf(self, pdf_file) -> List[TabellenZeile]:
        """Parst die PDF und extrahiert alle Zeilen als TabellenZeilen"""
        zeilen = []
        with pdfplumber.open(pdf_file) as pdf:
            for page in pdf.pages:
                text = page.extract_text()
                if text:
                    zeilen.extend(self._parse_text_zeilenweise(text))
        return zeilen
    
    def _parse_text_zeilenweise(self, text: str) -> List[TabellenZeile]:
        """Extrahiert Zeile für Zeile und teilt in linke/mittlere Spalte"""
        zeilen = []
        lines = text.split('\n')
        ...
        for line in lines:
            ...
            # Aufteilung in bis zu drei Spalten (Tabulator oder mehrere Leerzeichen)
            parts = re.split(r'\t|  +', line)
            if len(parts) >= 3:
                links = parts[0].strip()
                mitte = parts[1].strip()
                rechts = parts[2].strip() if len(parts) > 2 else ""
            elif len(parts) == 2:
                links = parts[0].strip()
                mitte = parts[1].strip()
                rechts = ""
            else:
                # Falls nur eine Spalte, versuchen wir zu erraten, ob es links oder mitte ist
                # Wenn es mit einer Zahl beginnt, ist es wahrscheinlich die linke Spalte
                if self.num_pattern.match(parts[0]):
                    links = parts[0].strip()
                    mitte = ""
                    rechts = ""
                else:
            ...
            # Prüfen, ob es eine neue Vokabel ist (beginnt mit Nummer)
            ist_neu = bool(self.num_pattern.match(links))
            
            zeilen.append(TabellenZeile(
                linke_spalte=links,
                mittlere_spalte=mitte,
                rechte_spalte=rechts,
                ist_neue_vokabel=ist_neu
            ))
    ...
    def fuelle_bedeutungen(self, zeilen: List[TabellenZeile]) -> List[TabellenZeile]:
        """
        Sucht für jede Zeile die Bedeutung:
        - Bei Zeilen mit Nummer: Suchwort = Hauptwort aus linker Spalte
        - Bei Zeilen ohne Nummer: Suchwort = Wort aus mittlerer Spalte
        """
        aktuelles_hauptwort = ""
        
        for zeile in zeilen:
            if zeile.ist_neue_vokabel:
                # Neues Hauptwort aus linker Spalte extrahieren
                griechwoerter = self.greek_pattern.findall(zeile.linke_spalte)
                if griechwoerter:
                    # Hauptwort bestimmen (erstes Wort, das KEIN Artikel ist)
                    for w in griechwoerter:
                        if w not in self.artikel:
                            aktuelles_hauptwort = w
                            break
                    if not aktuelles_hauptwort and griechwoerter:
                        aktuelles_hauptwort = griechwoerter[0]
                
                # Bedeutung für Hauptwort suchen
                if aktuelles_hauptwort:
                    bedeutungen, gefunden, _ = self.omega.finde_bedeutung(aktuelles_hauptwort)
                    if gefunden:
                ...
                # Zeile ohne Nummer: Suchwort aus mittlerer Spalte
                suchwort = zeile.mittlere_spalte.strip()
                if suchwort:
                    # Extrahiere das erste griechische Wort
                    griechwoerter = self.greek_pattern.findall(suchwort)
                    if griechwoerter:
                        suchwort = griechwoerter[0]
...
# ============================================
# PDF-GENERATOR MIT GRIECHISCHER SCHRIFT
# ============================================
...
class PDFGenerator:
    ...
    def erstelle_pdf(zeilen: List[TabellenZeile]) -> BytesIO:
        ...
        # Griechische Schriftart registrieren (DejaVuSans.ttf muss im Verzeichnis liegen)
        try:
            pdfmetrics.registerFont(TTFont('DejaVu', 'DejaVuSans.ttf'))
            schrift = 'DejaVu'
        except:
            # Fallback auf Helvetica (dann erscheinen evtl. wieder Quadrate)
            schrift = 'Helvetica'
            st.warning("⚠️ Schriftart 'DejaVuSans.ttf' nicht gefunden. Bitte herunterladen und ins App-Verzeichnis legen.")
        ...
        # Tabellenkopf
        data = [
            [Paragraph('Linke Spalte', kopf_stil),
             Paragraph('Mittlere Spalte', kopf_stil),
             Paragraph('Deutsch', kopf_stil)]
        ]
        ...
        table = Table(data, colWidths=[4.5*cm, 5.5*cm, 5*cm])
        table.setStyle(TableStyle([
            ('BACKGROUND', (0,0), (-1,0), colors.grey),
            ('TEXTCOLOR', (0,0), (-1,0), colors.whitesmoke),
            ('ALIGN', (0,0), (-1,-1), 'LEFT'),
            ('VALIGN', (0,0), (-1,-1), 'TOP'),
            ('FONTNAME', (0,0), (-1,0), schrift),
            ('FONTSIZE', (0,0), (-1,0), 11),
...
def main():
    ...
    st.markdown("Lade eine Vokabelliste im PDF-Format hoch. Die App ergänzt die deutschen Bedeutungen in der rechten Spalte.")
    
    with st.sidebar:
        st.header("ℹ️ Info")
        st.markdown("""
        **So funktioniert die Suche:**  
        - Zeilen mit **Nummer** → Bedeutung des Hauptworts (linke Spalte).  
        - Zeilen **ohne Nummer** → Bedeutung des Wortes in der mittleren Spalte (z.B. μᾶλλον, μάλιστα).  

        **Wichtig:** Damit griechische Zeichen im PDF korrekt dargestellt werden, muss eine Schriftart wie `DejaVuSans.ttf` im App-Verzeichnis liegen.  
        [Download DejaVuSans.ttf](https://dejavu-fonts.github.io/Download.html)
        """)
        st.markdown("---")
    ...
    if uploaded_file:
        ...
        if zeilen:
            ...
            col2.metric("Bedeutungen gefunden", gefunden, f"{gefunden/len(zeilen)*100:.0f}%")
            ...
            for i, z in enumerate(zeilen):
                with st.expander(f"Zeile {i+1}: {z.mittlere_spalte[:30] if z.mittlere_spalte else '(leer)'}...", expanded=False):
                    col_a, col_b = st.columns([1,2])
                    with col_a:
                        st.markdown(f"**Linke Spalte:** {z.linke_spalte}")
                        st.markdown(f"**Mittlere Spalte:** {z.mittlere_spalte}")
                        st.markdown(f"**Neue Vokabel:** {'✅' if z.ist_neue_vokabel else '❌'}")
                    with col_b:
                        aktuell = z.rechte_spalte
                        neu = st.text_area("Bedeutung", value=aktuell, key=f"korr_{i}", height=60)
                        if neu and neu != aktuell:
                            z.rechte_spalte = neu
                            st.success("✓ korrigiert")
                    korrigierte.append(z)
            ...
            with col2:
                if st.button("📊 CSV exportieren"):
                    df = pd.DataFrame([{
                        ...
                        'Neue Vokabel': z.ist_neue_vokabel
                    } for z in korrigierte])
            ...
            preview_df = pd.DataFrame([{
                'Links': z.linke_spalte[:30] + ('…' if len(z.linke_spalte) > 30 else ''),
                'Mitte': z.mittlere_spalte[:30] + ('…' if len(z.mittlere_spalte) > 30 else ''),
                'Bedeutung': z.rechte_spalte[:30] + ('…' if len(z.rechte_spalte) > 30 else ''),
                'Status': '✅' if z.rechte_spalte and z.rechte_spalte != "?" else '❌'
            } for z in korrigierte[:10]])
            st.dataframe(preview_df, use_container_width=True)
            if len(korrigierte) > 10:
                st.caption(f"*… und {len(korrigierte)-10} weitere Zeilen*")
        else:
            st.warning("⚠️ Keine Zeilen gefunden. Bitte überprüfe das Format.")
    else:
