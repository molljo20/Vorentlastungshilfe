...
@dataclass
class TabellenZeile:
    """Eine Zeile der extrahierten Tabelle (entspricht einer Zeile im PDF)"""
    linke_spalte: str      # Inhalt der linken Spalte (kann leer sein)
    mittlere_spalte: str   # Inhalt der mittleren Spalte (Stammform / Zusatz)
    rechte_spalte: str = "" # Wird später mit Bedeutung gefüllt
    position: int = 0       # Zeilennummer im Original (für Sortierung)
    hauptwort: str = ""     # Das zugehörige Grundwort (aus der ersten Zeile eines Blocks)

# ============================================
# OMEGA-WORTSCHATZ (VOLLSTÄNDIG, 800+ Einträge)
# ============================================
...
class OmegaWortschatz:
    ...
    def _build_synonyme(self):
        """Erweiterte Synonyme für Flexionsformen und häufige Varianten"""
        self.synonyme = {
            # Komparative / Superlative zu ἀγαθός
            "ἀμείνων": "ἀγαθός",
            "ἄμεινον": "ἀγαθός",
            "ἄριστος": "ἀγαθός",
            "κρείττων": "ἀγαθός",
            ...
            "βέλτιον": "ἀγαθός",
            "βέλτιστος": "ἀγαθός",
            # Komparative / Superlative zu κακός
            "χείρων": "κακός",
            "χεῖρον": "κακός",
            "χείριστος": "κακός",
            "κάκιστος": "κακός",
            # Komparative / Superlative zu μέγας
            "μείζων": "μέγας",
            "μεῖζον": "μέγας",
            "μέγιστος": "μέγας",
            # Komparative / Superlative zu μικρός / ὀλίγος
            "ἐλάττων": "μικρός",
            "ἔλαττον": "μικρός",
            "ἐλάχιστος": "μικρός",
            # Spezielle Formen aus deiner Vokabelliste
            "μᾶλλον": "μάλα",
            "μάλιστα": "μάλα",
            "οἱ πολλοί": "πολύς",
            "πλείων": "πολύς",
    ...
    def finde_bedeutung(self, wort: str) -> Tuple[Optional[List[str]], bool, str]:
        """Sucht nach dem Wort oder seinen Synonymen und gibt Bedeutung(en) zurück"""
        suchbegriff = wort.strip()
        # Direkte Suche
        ...
        if suchbegriff in self.synonyme:
            ...
            if hauptform in self.vocab_dict:
                ...
                bedeutungen = [b.strip() for b in bedeutung.split(';')]
                return bedeutungen, True, f"{stammformen} {zusatz} [zu {hauptform}]".strip()
        # Unscharfe Suche (ohne Akzente)
        suchbegriff_no = self._remove_accents(suchbegriff)
        for key in self.vocab_dict:
            if self._remove_accents(key) == suchbegriff_no:
...
# ============================================
# PDF-PARSER MIT TABELLENERKENNUNG
# ============================================

class VokabelPDFParser:
    def __init__(self):
        self.omega = OmegaWortschatz()
        # Griechische Buchstaben (inkl. akzentuierte)
        self.greek_pattern = re.compile(r'[\u0370-\u03FF\u1F00-\u1FFF]+')

    def parse_pdf(self, pdf_file) -> List[TabellenZeile]:
        """Extrahiert die Tabelle aus dem PDF und gibt eine Liste von Zeilen zurück."""
        zeilen = []
        with pdfplumber.open(pdf_file) as pdf:
            for seite in pdf.pages:
                # Tabelle extrahieren (falls Linien vorhanden, sonst mit Textstrategie)
                ...
                if tabelle:
                    for zeile in tabelle:
                        if len(zeile) >= 3:
                            links = (zeile[0] or "").strip()
                            mitte = (zeile[1] or "").strip()
                            rechts = (zeile[2] or "").strip()
                            if links or mitte:  # Leerzeilen ignorieren
                                zeilen.append(TabellenZeile(
                                    linke_spalte=links,
                                    mittlere_spalte=mitte,
                                    rechte_spalte=rechts
                                ))
                else:
                    # Fallback: Text manuell parsen (falls Tabelle nicht erkannt)
                    text = seite.extract_text()
                    if text:
                        zeilen.extend(self._parse_text_fallback(text))
        # Nach Position sortieren? Wir haben keine Position, aber die Reihenfolge sollte stimmen.
        return zeilen

    def _parse_text_fallback(self, text: str) -> List[TabellenZeile]:
        """Einfacher Parser, falls extract_table fehlschlägt."""
        zeilen = []
        lines = text.split('\n')
        for line in lines:
            line = line.strip()
            if not line:
                continue
            # Versuche, drei Spalten zu erkennen (durch Tabulator oder Mehrfach-Leerzeichen)
            parts = re.split(r'\t|  +', line)
            if len(parts) == 3:
                zeilen.append(TabellenZeile(parts[0], parts[1], parts[2]))
            elif len(parts) == 2:
                # Nur links und mitte
                zeilen.append(TabellenZeile(parts[0], parts[1], ""))
            elif len(parts) == 1:
                # Nur eine Spalte (z.B. Fortsetzung der mittleren Spalte)
                zeilen.append(TabellenZeile("", parts[0], ""))
            else:
                # Ignorieren
                pass
        return zeilen

    def fuelle_bedeutungen(self, zeilen: List[TabellenZeile]) -> List[TabellenZeile]:
        """Sucht für jede Zeile die Bedeutung des Wortes in der mittleren Spalte."""
        aktuelles_hauptwort = ""
        for zeile in zeilen:
            # Wenn linke Spalte nicht leer ist, extrahieren wir das Hauptwort (erstes griechisches Wort)
            if zeile.linke_spalte:
                griechwoerter = self.greek_pattern.findall(zeile.linke_spalte)
                if griechwoerter:
                    aktuelles_hauptwort = griechwoerter[0]
            zeile.hauptwort = aktuelles_hauptwort

            # Bedeutung für das Wort in der mittleren Spalte suchen
            suchwort = zeile.mittlere_spalte.strip()
            if suchwort:
                bedeutungen, gefunden, _ = self.omega.finde_bedeutung(suchwort)
                if gefunden:
                    zeile.rechte_spalte = '; '.join(bedeutungen)
                else:
                    # Wenn nicht gefunden, vielleicht ist es eine Phrase, die zum Hauptwort gehört?
                    # Z.B. "οἱ πολλοί" – versuchen wir, es über Synonyme zu finden
                    if suchwort in self.omega.synonyme:
                        # Nochmal mit Synonym
                        bedeutungen, gefunden, _ = self.omega.finde_bedeutung(suchwort)
                        if gefunden:
                            zeile.rechte_spalte = '; '.join(bedeutungen)
...
# ============================================
# PDF-GENERATOR MIT MEHRZEILIGEN ZELLEN
# ============================================

class PDFGenerator:
    @staticmethod
    def erstelle_pdf(zeilen: List[TabellenZeile]) -> BytesIO:
        buffer = BytesIO()
        doc = SimpleDocTemplate(buffer, pagesize=A4, leftMargin=1.5*cm, rightMargin=1.5*cm)
        story = []
        styles = getSampleStyleSheet()
        # Eigenen Stil für Zellen mit kleinerer Schrift und Zeilenumbruch
        zellen_stil = ParagraphStyle(
            'ZellenStil',
            parent=styles['Normal'],
            fontSize=9,
            leading=11,
            wordWrap='CJK'  # Erlaubt Umbruch bei langen Wörtern
        )

        # Kopfzeile
        data = [['', 'Griechisch', 'Deutsch']]  # Die erste Spalte bleibt leer (wir zeigen keine Nummern extra)
        for z in zeilen:
            # Linke Spalte: Wenn vorhanden, anzeigen, sonst leer
            links = Paragraph(z.linke_spalte, zellen_stil) if z.linke_spalte else ""
            mitte = Paragraph(z.mittlere_spalte, zellen_stil) if z.mittlere_spalte else ""
            rechts = Paragraph(z.rechte_spalte, zellen_stil) if z.rechte_spalte else ""
            data.append([links, mitte, rechts])
        ...
        story.append(Paragraph("Vokabeln basierend auf dem Omega-Wortschatz (CC BY-NC-SA 4.0)", styles['Italic']))
        doc.build(story)
        buffer.seek(0)
        return buffer
...
def main():
    ...
    st.title("🏛️ Altgriechisch Vokabeltrainer")
    st.markdown("Lade eine Vokabelliste im PDF-Format hoch (mit drei Spalten: Nummer+Hauptwort, Stammformen, leer). Die App ergänzt automatisch die deutschen Bedeutungen aus dem Omega-Wortschatz.")

    with st.sidebar:
        st.header("ℹ️ Info")
        st.markdown("""
        **Erwartetes Format:**  
        Das PDF sollte eine Tabelle mit drei Spalten enthalten:
        - **Linke Spalte:** Nummer und Hauptwort (z.B. `1ὁ ἀνήρ (1)`)
        - **Mittlere Spalte:** Stammformen, Flexionen (kann mehrzeilig sein)
        - **Rechte Spalte:** leer (wird gefüllt)

        Die App extrahiert die Tabelle und sucht für jedes Wort in der mittleren Spalte die Bedeutung.
        """)
        st.markdown("---")
        st.caption("Omega-Wortschatz © Ulrich Gebhardt, Freiburg - CC BY-NC-SA 4.0")
    ...
    if uploaded_file:
        with st.spinner("Analysiere PDF und suche Bedeutungen ..."):
            ...
            zeilen = parser.parse_pdf(uploaded_file)
            zeilen = parser.fuelle_bedeutungen(zeilen)

        if zeilen:
            st.success(f"✅ {len(zeilen)} Zeilen verarbeitet.")

            # Statistik
            gefunden = sum(1 for z in zeilen if z.rechte_spalte and z.rechte_spalte != "?")
            col1, col2, col3 = st.columns(3)
            col1.metric("Zeilen insgesamt", len(zeilen))
            col2.metric("Bedeutungen gefunden", gefunden)
            col3.metric("Nicht gefunden", len(zeilen)-gefunden)

            # Manuelle Korrektur (vereinfacht)
            st.markdown("---")
            st.subheader("✏️ Korrektur (optional)")
            korrigierte_zeilen = []
            for i, z in enumerate(zeilen):
                with st.expander(f"Zeile {i+1}: {z.mittlere_spalte[:30]}...", expanded=False):
                    st.markdown(f"**Linke Spalte:** {z.linke_spalte}")
                    st.markdown(f"**Mittlere Spalte:** {z.mittlere_spalte}")
                    aktuell = z.rechte_spalte
                    neu = st.text_input("Bedeutung", value=aktuell, key=f"korr_{i}")
                    if neu != aktuell:
                        z.rechte_spalte = neu
                    korrigierte_zeilen.append(z)
            ...
            with col1:
                if st.button("📄 PDF erstellen", type="primary"):
                    pdf_data = PDFGenerator.erstelle_pdf(korrigierte_zeilen)
                    ts = datetime.now().strftime("%Y%m%d_%H%M%S")
                    st.download_button(
                        label="⬇️ PDF herunterladen",
                        data=pdf_data,
                        file_name=f"vokabeln_{ts}.pdf",
                        mime="application/pdf"
                    )
            with col2:
                if st.button("📊 CSV exportieren"):
                    df = pd.DataFrame([{
                        'Linke Spalte': z.linke_spalte,
                        'Mittlere Spalte': z.mittlere_spalte,
                        'Bedeutung': z.rechte_spalte
                    } for z in korrigierte_zeilen])
                    csv = df.to_csv(index=False, encoding='utf-8-sig')
                    ts = datetime.now().strftime("%Y%m%d_%H%M%S")
                    st.download_button(
                        label="⬇️ CSV herunterladen",
                        data=csv,
                        file_name=f"vokabeln_{ts}.csv",
                        mime="text/csv"
                    )
            ...
            preview_df = pd.DataFrame([{
                'Links': z.linke_spalte[:30],
                'Mitte': z.mittlere_spalte[:30],
                'Bedeutung': z.rechte_spalte[:30]
            } for z in korrigierte_zeilen[:10]])
            st.dataframe(preview_df, use_container_width=True)
            if len(korrigierte_zeilen) > 10:
                st.caption(f"*… und {len(korrigierte_zeilen)-10} weitere Zeilen*")
        else:
            st.warning("⚠️ Keine Tabellenzeilen gefunden. Bitte überprüfe das Format.")
    else:
